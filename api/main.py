...
@app.post("/api/stt")
async def stt(audio: UploadFile = File(...)) -> dict[str, str]:
    """Распознаёт речь из аудио-файла (любой формат, что понимает ffmpeg).

    Используется фронтом для голосового ввода: браузер пишет звук через
    MediaRecorder (обычно webm/opus), кидает сюда, мы декодируем через
    ffmpeg в 16kHz mono PCM и прогоняем через офлайн Vosk-модель
    (русский small).
    """
    ...
    if len(raw) > MAX_AUDIO_BYTES:
        raise HTTPException(413, "audio too large (max 16 MB)")
    ...
    if proc.returncode != 0:
        msg = proc.stderr[-400:].decode(errors="replace")
        log.warning("ffmpeg decode failed: %s", msg)
        raise HTTPException(400, f"audio decode failed: {msg}")
